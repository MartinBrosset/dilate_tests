...
traffic_train = np.array(pd.read_table(DATA_PATH + "Traffic2/TRAFFIC2_TRAIN.tsv"))[:, :, np.newaxis]
traffic_test = np.array(pd.read_table(DATA_PATH + "Traffic2/TRAFFIC2_TEST.tsv"))[:, :, np.newaxis]
...
train_model(net_gru_dilate,loss_type='dilate',learning_rate=0.0005, epochs=400, gamma=gamma, alpha = 0.2, print_every=5)
...
train_model(net_gru_mse,loss_type='mse',learning_rate=0.0005, epochs=300, gamma=gamma, print_every=5)
